...
namespace TP3
{
    public class MovieCollection
    {
        ...
        public void Launch()
        {
            ...
            // Builder pattern
            var oldestFilm = Movies.OrderBy(movies => movies.ReleaseDate).First().Title;
            Console.WriteLine("\n The oldest movie is : " + oldestFilm);
            
            
            // Count all movies.
            var numberOfFilm = Movies.Count();
            Console.WriteLine("\n The total number of movie is : " + numberOfFilm);
            
            //Count all movies with the letter e. at least once in the title
            var eLetterPresent = Movies.Where(movies => movies.Title.Contains("e")).Count();
            Console.WriteLine("\n number of films with e letter inside : " + eLetterPresent);
            ...
            Console.WriteLine("\n Here is the number of films with letter f in the title : " + counter);    
            
            
            //Display the title of the film with the higher budget.
            var maxBudget = Movies.OrderBy(movies => movies.Budget).Last().Title;
            Console.WriteLine("\n the title with the higher budget : " + maxBudget);
            
            //Display the title of the movie with the lowest box office
            var minBoxOffice = Movies.OrderBy(movies => movies.BoxOffice).First().Title;
            Console.WriteLine("\n the title with the lowest box office : " + minBoxOffice);
            
            //Order the movies by reversed alphabetical order and print the first 11 of the list.
            var elevenFirstFilm = Movies.OrderByDescending(movies => movies.Title).Take(11);
            Console.WriteLine("\n Order the movies by reversed alphabetical order and print the first 11 of the list : ");
            foreach (var film in elevenFirstFilm)
            {
            ...
            var movieBefore1980 = Movies.Where(movies => movies.ReleaseDate < date).Count();
            Console.WriteLine("\n Here is the number of movies made before 1980 : " + movieBefore1980);
            
            //Display the average running time of movies having a vowel as the first letter.
            string vowels = "AEIOUY";
            var firstLetterVowel = Movies.Where(movies => vowels.Contains(movies.Title[0])).Count();
            var count = Movies.Count();
            Console.WriteLine("\n Here is the average running time of movies having a vowel as the first letter : " + ((firstLetterVowel / (float)count) * 100));
           
            //Print all movies with the letter H or W in the title, but not the letter I or T.
            Console.WriteLine("\n Here are all movies with the letter H or W in the title, but not the letter I or T : ");
            string letterAllowed = "HhWw";
            string letterNotAllowed = "IiTt";
            
            var titleWithLetter = Movies.Where(movies => (movies.Title.Contains("H") || movies.Title.Contains("W")
                                                        || movies.Title.Contains("h") || movies.Title.Contains("w")) &&
                                                        !(movies.Title.Contains("I")|| movies.Title.Contains("i") 
                                                            || movies.Title.Contains("t")  || movies.Title.Contains("T")));
            
            foreach (var film in titleWithLetter)
            {
                Console.WriteLine(film.Title);
            }
